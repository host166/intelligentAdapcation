...
var intelligent = (function(){
    ...
    F.prototype.prefixEvent = function(){   //处理css3的前缀属性问题的方法
        ...
        if( !!this.option.prefix ){ //为真的时候才执行
            ...
            var repalce = css_data_usable.replace(/(animation|keyframes|transform|transition|perspective|linear-gradient)+(\:|([\-]+(?:\w))|(\s\:)|\s)/gi,"<=regPrefixName=>").split("<=regPrefixName=>");
            var match   = css_data_usable.match(/(animation|keyframes|transform|transition|perspective|linear-gradient)+(\:|([\-]+(?:\w))|(\s\:)|\s)/gi);
        ...
        if( this.option.backUrl.indexOf("http") != -1 ){    //http字符表示存在
        ...
        //屏幕翻转的时候或者改变大小的时候进行适配处理
        this.addResizeEvent("recalculate",function(){
            _this.pixedAllAttribute();
        });
    };
    
    F.prototype.backgroundAttribute = function(){   //处理背景图片连接代码拆分问题
        var _this = this;
        var css_data_usable = this.cssData.usable;
        ...
        if( !!match ){
            ...
            this.cssData.usable   = css_data_usable;
            this.cssData.original = css_data_usable;    //保存不变的准备处理数值的css代码到变量中
        };
    };
    
    F.prototype.backAbsoluteAddress = function(){   //处理北京中需要添加背景图片的绝对地址的时候
        var _this = this;
        var css_data_usable = this.cssData.usable;
        //获取有背景图片的属性
    ...
    F.prototype.pixedAllAttribute = function(){
        var win = $(window);
        var designSize = this.option.designSize;
        var widthHeight = win.width()/win.height();

        this.windowWidth = !!this.option.proportion ? (designSize < widthHeight ? designSize * win.height() : win.width()) : win.width();   //窗口尺寸获得

        var _this = this;
        var css_data_usable = this.cssData.usable = this.cssData.original;  //要处理的可用于处理的css代码变量
        ...
        if( !!match ){    //存在像素值的时候 才开始加工
            ...
            $.each(repalce,function(i,val){
                if(i == match.length){
                    css_data_usable += val; //把px值重新组装到占位符
                }else{
                    //像素值
                    ...
                    //判断是否进行四舍五入模式
                    var result = (_this.option.rounding?Math.round( min_value ):min_value)+"px";
                    css_data_usable += val + result; //把px值重新组装到占位符
                };
            });
    ...
    F.prototype.mediaCodeAppend = function(){
        ...
        *   6.计算的时候于arr[i].mediaPx进行比较计算的
        ...
        var repalce = css_data_usable.replace(/(<=mea=>)/gi,"<=凸=>").split("<=凸=>");
        if( !!repalce ){
            css_data_usable = "";

            $.each(repalce,function(i,val){
                if( i > 0 ){
                    var repalceIn = val.replace(/(<=m=>)/gi,"<=zwf=>").split("<=zwf=>");
                    //循环处理每个数组中的占位符
                    var str = "";
                    $.each(repalceIn,function(j,elem){
                        if( j == repalceIn.length-1 ){
                            str += elem;
                        }else{
                            var resultValue = "";
                            if( j == 0 ){
                                resultValue += arr[i-1].oMedia;
                            }else{
                                var result  = (parseInt(arr[i-1].medInPx[j-1]) / parseInt(arr[i-1].mediaPx) * _this.windowWidth);
                                resultValue += (_this.option.rounding?Math.round( result ):result) + "px";
                            };
                            str += elem + resultValue;
                        };
                    });
                    css_data_usable += str;
                }else{
    ...
    F.prototype.newCSSElement = function(){ 
        var _this = this;
        var css_data_usable = !!this.option.allCodeShow ? (this.cssData.startTag+this.cssData.usable+this.cssData.endTag) : this.cssData.usable;    //allCodeShow是否开启
        ...
        var appendFunc = function() {
            ...
            if( !!_this.option.callback ){
                _this.option.callback();    //新的样式创建完毕之后的回调事件
            };
        };
    ...
    F.prototype.addResizeEvent = function(name,callback){    //窗口变化时或者设备翻转时执行适配展示的方法
        var _this = this;

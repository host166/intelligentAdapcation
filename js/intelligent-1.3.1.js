...
var intelligent = {
            ...
            $.each(media_in_pixed,function(i){
                ...
                if(i > 0){
                    ...
                    $.each(px_result,function(j){
                        ...
                        //结算值
                        var result_val = px_I / parseInt(media_pixed[i-1]) * _this.windowWidth;
                        //最小值处理
                        var min_value  = (result_val<1&&result_val>0)?result_val=1:result_val;
                        //分流判断是否使用四舍五入模式 true开启 false关闭
                        var result = _this.option.rounding?Math.round( result_val ):result_val;
    ...
    pixed_factory : function(){ //像素工厂
        ...
        if(!!pixed_match){
            ...
            $.each(pixed_placeholder,function(i,elem){  //i:个数 elem:对象
                ...
                //结算的值
                var result_val = (aI / _this.option.design) * _this.windowWidth;
                //最小值处理
                var min_value  = (result_val<1&&result_val>0)?result_val=1:result_val;
                //判断是否进行四舍五入模式
                var result = _this.option.rounding?Math.round( result_val )+"px":result_val+"px";  //加工处理
    ...
    /*border_factoryIn_pixed : function(){    //border处理工厂
        var _this = this;
        var cssData = this.obtain_usable_data;

        //创建border中像素的占位符
        var borderStringPixed = cssData.replace(/(border:|border\-top:|border\-right:|border\-bottom:|border\-left:)+([a-zA-Z0-9\.\-\s\#])+\;/gi,"&BOR&").split("&BOR&");
        //得到border的属性
        var borderMatchPixed = cssData.match(/(border:|border\-top:|border\-right:|border\-bottom:|border\-left:)+([a-zA-Z0-9\.\-\s\#])+\;/gi).join("");

        if(!!borderMatchPixed){
            var bsp = borderMatchPixed.replace(/([\d\.]+)(?:px)/gi,"&PX&").split("&PX&");
            var bmp = borderMatchPixed.match(/([\d\.]+)(?:px)/gi);
            console.log(bmp)
        };
    },*/

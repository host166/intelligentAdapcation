...
var intelligent = (function(){
    ...
    F.prototype.prefixEvent = function(){   //处理css3的前缀属性问题的方法
        ...
        if( !!this.option.prefix ){ //为真的时候才执行
            ...
            var repalce = css_data_usable.replace(/(animation|keyframes|transform|transition|perspective|linear-gradient)/gi,"<=regPrefixName=>").split("<=regPrefixName=>");
            var match   = css_data_usable.match(/(animation|keyframes|transform|transition|perspective|linear-gradient)/gi);
...
$(function(){
    intelligent.init({
        ...
        design   : 640,
        fsMin    : 11,
        backUrl  : "http://localhost/intelligent/images/"
    });
});
